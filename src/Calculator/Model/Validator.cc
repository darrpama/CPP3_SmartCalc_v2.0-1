...
namespace s21
{
  ...
  void Validator::IsNotCorrect(const std::string &inputString)
  {
    ...
    if (!e_check)
    {
      ...
      // std::cout << brCheck << plMin << nCheck << tCheck << fCheck << bOpCheck << std::endl;
    }
  ...
  bool Validator::IsPlusMinusNotCorrect(const std::string &inputString)
  {
    ...
    for (size_t i = 0; i < inputString.size(); i++)
    {
      ...
      {
        if (i + 1 >= inputString.size() || NotContains(ex_str, inputString[i + 1]))
        {
          return true;
        }
        else if (i != 0 && NotContains(allowed_before_plus_minus_, inputString[i - 1]))
        {
          return true;
        }
      }
  ...
  bool Validator::IsFooNotCorrect(const std::string &inputString)
  {
    for (size_t i = 0; i < inputString.size(); i++)
    {
      if (inputString[i] == 'a' && (inputString[i + 1] == 's' || inputString[i + 1] == 'c') &&
          ((inputString[i + 2] != 'i' && inputString[i + 2] != 'o') || (inputString[i + 3] != 'n' && inputString[i + 3] != 's') || inputString[i + 4] != '(' || !IsDigitOrPm(inputString[i + 5])))
      {
        return true;
      }
